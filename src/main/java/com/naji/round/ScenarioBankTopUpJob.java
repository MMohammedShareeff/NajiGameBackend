...
public class ScenarioBankTopUpJob {
    ...
    public void addFreshScenarios() {
        for (String lang : GameLanguage.SUPPORTED) {
            for (int slot = 1; slot <= ScenarioBankService.SLOT_COUNT; slot++) {
                try {
                    if (bank.leastUsedCount(slot, lang) < MIN_USES_BEFORE_TOP_UP) {
                        continue;
                    }
                    ScenarioTheme theme = ScenarioThemes.forRound(slot);
                    List<String> recent = bank.recentTexts(slot, lang, RECENT_TEXTS_SHOWN_TO_AI);
                    String scenario = openAiService.getScenario(theme, slot, ScenarioBankService.SLOT_COUNT, recent, lang);
                    bank.add(slot, lang, GameLanguage.themeLabel(lang, slot, theme), scenario);
                    logger.info("Added a new {} scenario to slot {}", lang, slot);
                } catch (RuntimeException ex) {
                    logger.warn("Could not add a {} scenario to slot {}: {}", lang, slot, ex.getMessage());
                }
            }
        }
    }
